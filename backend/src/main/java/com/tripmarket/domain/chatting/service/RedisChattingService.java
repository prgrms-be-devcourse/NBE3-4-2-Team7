...
public class RedisChattingService {
	...
	// 채팅방에 유저 추가
	public void addUserChattingRoom(String roomId, String userEmail) {
		redisTemplate.opsForSet().add("chatRoom:" + roomId, userEmail);
	}

	// 채팅방에서 유저 제거
	public void deleteUserChattingRoom(String roomId, String userEmail) {
		redisTemplate.opsForSet().remove("chatRoom:" + roomId, userEmail);
	}

	// 채팅방에 접속 중인 유저 목록 가져오기
	public Set<Object> getUsersByChattingRoom(String roomId) {
		return redisTemplate.opsForSet().members("chatRoom:" + roomId);
	}

	// 읽지 않은 메시지 수 증가
	public void addUnreadCount(String roomId, String receiver) {
